...
fn shape_text_with_font(
    ...
    fontdb.with_face_data(font.id, |font_data, face_index| -> Option<Vec<Glyph>> {
        use harfruzz::{Feature, ShaperData, ShaperInstance, Tag, UnicodeBuffer, Variation};

        let hr_font = harfruzz::FontRef::from_index(font_data, face_index).ok()?;
        ...
        for run in runs.iter() {
            ...
            let direction = if ltr {
                harfruzz::Direction::LeftToRight
            } else {
                harfruzz::Direction::RightToLeft
            };
            ...
            let shaper = shaper_data
                ...
                // Set the desired point size for trak
